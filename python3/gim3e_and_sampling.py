if __name__ == '__main__':
 ...
 conditions_of_interest=list(model_dict.keys()) #Conditions that will be analyzed with GIMME
 ...
 for sample in sorted(conditions_of_interest):
    ...
    for reaction in model.reactions: 
        ...
        try:
           ...
           equation=""
        out_row={"Reaction id":rid,"Reaction name":reaction.name,"Reaction":reaction.reaction,"Reaction Stoichiometry":equation,"Optimal solution":solution_dict.get(rid),"Minimum":fva_min,"Maximum":fva_max,"Reaction expression":reaction_expression_dict.get(reaction.id),"Sampling mean":mean,"Sampling SD":std}   
    ...
    output_sheet=pd.DataFrame(out_rows)
    ...
    output_sheet.to_csv(out_prefix+".csv",index=False)
